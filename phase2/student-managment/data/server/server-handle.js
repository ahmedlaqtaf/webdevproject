...
//alot of work needed to be done here
...
coursesRouter.post('/', async (req, res) => {
  ...
  if (!course.title) return sendResponse(res, 400, null, 'Course title is required');
  ...
  sendResponse(res, status, result, !course.id ? 'Course created' : 'Course updated');
});
...
sectionsRouter.post('/', async (req, res) => {
  ...
  sendResponse(res, status, result, !section.id ? 'Section created' : 'Section updated');
});
...
registrationsRouter.get('/:id', async (req, res) => {
  ...
  if (!registration) return sendResponse(res, 404, null, 'Registration not found');
...
registrationsRouter.post('/', async (req, res) => {
  ...
  sendResponse(res, status, result, !registration.id ? 'Registration created' : 'Registration updated');
});
...
publishedCoursesRouter.get('/:id', async (req, res) => {
  ...
  if (!course) return sendResponse(res, 404, null, 'Published course not found');
...
publishedCoursesRouter.delete('/:id', async (req, res) => {
  const result = await publishedCoursesRepo.deletePublishedCourse(req.params.id);
  sendResponse(res, 200, null, result);
});
    ...
    if (middleware.route) {
      ...
      subRouter.stack.forEach((subMiddleware) => {
        if (subMiddleware.route) {
          const path = `/api${middleware.regexp.source.replace('/?', '')}${subMiddleware.route.path}`;
          const methods = Object.keys(subMiddleware.route.methods).map((m) => m.toUpperCase());
          endpoints.push({ methods, path });
        }
      });
    }
...
app.listen(PORT, () => {
  ...
  console.log('API handles CRUD operations for student management system entities.');
});